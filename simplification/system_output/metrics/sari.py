...
# This code is taken from https://github.com/cocoxu/simplification/blob/master/SARI.py
...
def get_references(references_folder, simplified_file):
    references_fps = []
    for ref_file in glob.glob(references_folder + "/*"):
        references_fps.append(open(ref_file))

    all_reference_sentences = []
    for _ in open(simplified_file):
        references = []
        for fp in references_fps:
            line = fp.readline().strip()
            references.append(line.lower())
        all_reference_sentences.append(references)

    return all_reference_sentences
...
def compute_sari(complex, reference, simplified):
    complex_sentences = ReadInFile(complex)
    reference_sentences = get_references(reference, simplified)
    simplified_sentences = ReadInFile(simplified)
    ...
    for i in range(len(simplified_sentences)):
        simplex = " ".join(simplified_sentences[i].split()[:80])
        keep, dels, add, final = SARIsent(complex_sentences[i].lower(), simplex.lower(), reference_sentences[i])
